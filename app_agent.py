...
async def call_protocol_api(status_agent: str, user: str, beneficiary: str, legacy: str, contact_id: str = None):
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(f"https://my-last-wish-api-df78085c0eca.herokuapp.com/protocols/{status_agent}", json=data)
        print(response.json())
...
async def generate_memorial(user: str,beneficiary: str, api: TelegramAPI) -> str:
    ...
    async with httpx.AsyncClient() as client:
        ...
        await api.send_msg(beneficiary, memorial_message)
        await api.send_image(beneficiary, image.url, "memorial")
